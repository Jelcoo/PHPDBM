            ...
            <?php foreach ($tableRows['data'] as $row) { ?>
                <tr class="row-<?php echo isset($primaryKey) ? $row[$primaryKey] : 'unknown'; ?>">
                    ...
                    <?php foreach ($tableColumns as $column) { ?>
                        ...
                        <td class="align-middle text-truncate text-truncate-width <?php echo $value === null || empty($value) ? 'fst-italic text-danger-emphasis' : ''; ?> field-<?php echo $column['Field']; ?>"><?php echo ($value === null ? 'NULL' : empty($value)) ? 'EMPTY' : htmlspecialchars($value); ?></td>
                    <?php } ?>
                </tr>
            <?php } ?>
